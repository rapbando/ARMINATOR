...
bool L3State ;           // current state of the R2 button 
bool lastL3State = 0;     // previous state of the R2 button
...
void loop(){
  ...
  L2State = (Xbox.getButtonPress(L2)); 
   ...
   // if the current state is TRUE then the button went from off to on:
    if (L2State  == true)  {
      ...
      A_FWD(400); // set N of steps to move forward 2 laps, 200 steps are a shaft turn.

                           }   
   // on button release go back to reset the original position
   else  {
    
      A_BACK(400); // set N of steps to move backward 2 laps, 200 steps are a shaft turn.
 ...
 // read the pushbutton input from serial
  L3State = (Xbox.getButtonPress(R2));

   // compare the buttonState to its previous state
   if (L3State != lastL3State) {

   // if the current state is TRUE then the button went from off to on:
    if (L3State == true)  {
      
      A_BACK(400); // set N of steps to move backward 2 laps, 200 steps are is a shaft turn.
      
                          }   
    // on button release go back to reset the original position
   else  {
    
      A_FWD(400); // set N of steps to move forward 2 laps, 200 steps are is a shaft turn.
   ...
   // if the current state is TRUE then the button went from off to on:
    ...
    if (RHY_P_State == true)  {
      ...
      Y_BACK(400); // set N of steps to move backward 2 laps, 200 steps are a shaft turn.
      
                          }   
     // on hat release go back to reset the original position   
    else {
    
      Y_FWD(400); // set N of steps to move forward 2 laps, 200 steps are a shaft turn.
          }  
   ...
   // if the current state is TRUE then the button went from off to on:
    ...
    if (LHY_P_State == true)  {
      ...
      Z_BACK(400); // set N of steps to move backward 2 laps, 200 steps are a shaft turn.
      
                          }   
     // on hat release go back to reset the original position   
    else {
    
      Z_FWD(400); // set N of steps to move forward 2 laps, 200 steps are a shaft turn.
   ...
   // if the current state is TRUE then the button went from off to on:
    ...
    if (LHX_P_State == true)  {
      ...
      X_BACK(400); // set N of steps to move backward 2 laps,  200 steps are a shaft turn.
      
                          }   
    // on hat release go back to reset the original position    
    else {
    
      X_FWD(400); // set N of steps to move forward 2 laps,  200 steps are a shaft turn.
